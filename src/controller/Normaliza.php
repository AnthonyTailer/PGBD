...
if ($request == "regiao") {
  ...
  try {
    ...
    $query .= "UPDATE RECLAMACAO_DES RD SET IDCONSUMIDOR = (SELECT IDCONSUMIDOR FROM CONSUMIDOR CO NATURAL JOIN CIDADE CI
        WHERE CO.SEXO = RD.SEXO AND CO.FAIXAETARIA = RD.FAIXAETARIA AND CI.NOME = RD.CIDADE);";
  ...
  $query = "INSERT INTO AREA (DESCRICAO)
  ...
  $query = "INSERT INTO EMPRESA (NOMEFATASIA, IDSEGMENTO)
  ...
  $query = "INSERT INTO GRUPO (DESCRICAO) SELECT DISTINCT
   GRUPOPROBLEMA FROM RECLAMACAO_DES RD ORDER BY 1 ASC";
  ...
  try {
    ...
    $query = "INSERT INTO RECLAMACAO (IDCONSUMIDOR, ANO, MES, DATAABERTURA, DATARESPOSTA, DATAFINALIZACAO,
        ...
        TEMPORESPOSTA, E.IDEMPRESA, A.IDAREA, ASSUNTO, P.IDPROBLEMA, COMOCOMPROU, PROCUROUEMPRESA, RESPONDIDA,
        SITUACAO, AVALIACAO, NOTACONSUMIDOR 
           FROM RECLAMACAO_DES RD
           JOIN EMPRESA E ON RD.NOMEFANTASIA = E.NOMEFATASIA
