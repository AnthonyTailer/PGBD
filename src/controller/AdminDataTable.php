...
if ($request == "desnormalizada") {
    ...
    $query = "SELECT IDRECLAMACAO, CI.NOME AS CIDADE, ES.NOME AS UF, RG.NOME AS REGIAO, C.SEXO, C.FAIXAETARIA, ANO, MES, 
                DATAABERTURA, DATARESPOSTA, DATAFINALIZACAO, TEMPORESPOSTA, E.NOMEFANTASIA, S.DESCRICAO AS SEGMENTO, 
                A.DESCRICAO AS AREA, ASSUNTO, P.DESCRICAO AS PROBLEMA, COMOCOMPROU, PROCUROUEMPRESA, RESPONDIDA, SITUACAO, 
                AVALIACAO, NOTACONSUMIDOR 
                FROM RECLAMACAO R
                JOIN AREA A ON A.IDAREA = R.IDAREA 
                JOIN CONSUMIDOR C ON C.IDCONSUMIDOR = R.IDCONSUMIDOR
                JOIN CIDADE CI ON C.IDCIDADE = CI.IDCIDADE 
                JOIN ESTADO ES ON CI.IDESTADO = ES.IDESTADO 
                JOIN REGIAO RG ON ES.IDREGIAO = RG.IDREGIAO
